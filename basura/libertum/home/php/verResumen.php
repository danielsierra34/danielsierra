...
//INICIO PROCESAMINTO PRESTAMOS
 ...
 $cuantoPreste->valor=$objeto->valor;
  ...
  $queryc="select sum(case when deudores.status=3 then 1 else 0 end) as totalFinalizados,sum(case when status=3 then deudores.calificacion else 0 end) as totalCalificacion from deudores where idUsuarioDeudor=$idUsuario and status in (0,1,2,3)";
  $resultc = $mysqli->query($queryc);
  $objetoc= mysqli_fetch_object($resultc);
  $totalCalificacion=$objetoc->totalCalificacion;
  $totalFinalizados=$objetoc->totalFinalizados;
  $calificacion=$totalCalificacion/$totalFinalizados;
  
  
  
  //QUIEN ME DEBE. EN ESTE CASO YO SOY IDUSUARIO
  $query="select deudores.idUsuario,deudores.idUsuarioDeudor, sum(case when status=1 then deudores.valor else 0 end) as valor,sum(case when deudores.status=3 then 1 else 0 end) as totalFinalizados,sum(case when status=3 then deudores.calificacion else 0 end) as totalCalificacion, usuarios.NOMBRE from deudores LEFT JOIN usuarios on deudores.idUsuarioDeudor=usuarios.ID where deudores.idUsuario=$idUsuario and deudores.status in(0,1,3) group by deudores.idUsuarioDeudor";
	$result = $mysqli->query($query);
