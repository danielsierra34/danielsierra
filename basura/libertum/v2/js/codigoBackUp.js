		...
		success: function(respuesta) {
			...
			$.each(nodoDeudas, function(x, y) {
				...
				$("#listadoDeudas").append(`<div class="col-md-4">
									...
									<div class="card-header">${y.nombre}<a class="float-right" data-toggle="collapse" href="#bloque-deudas-${x}" role="button" aria-expanded="false" aria-controls="collapseExample">
...
<i class="far fa-edit despegado"></i>
				...
				$("#listadoPrestamos").append(`<div class="col-md-4">
							<div class="card bg-light mb-3">  									
								<div class="card-header">${y.NOMBRE}<a class="float-right" data-toggle="collapse" href="#bloque-prestamos-${x}" role="button" aria-expanded="false" aria-controls="collapseExample">
 								 <i class="fas fa-angle-right"></i>  </a><span class="span-valor">${formata(y.valor)}</span>
										</div>
			...
			$.each(nodoNotificaciones, function(x, y) {
				$("#listadoNotificaciones").append(`<div class="col-md-12">
  									<div class="card bg-light mb-3">
									<div class="card-header">${y.nombre} ${y.apellido} te presto <b>${formata(y.valor)}</b> en ${y.fecha_transaccion}
			...
			$.each(nodoRechazos, function(x, y) {
				$("#listadoRechazos").append(`<div class="col-md-12">
  									<div class="card bg-light mb-3">
									<div class="card-header">${y.nombre} ${y.apellido} rechazo tu prestamo por <b>${formata(y.valor)}</b> en ${y.fecha_transaccion}
										</div>
  									</div>
									</div>`)
			})
		...
		$.ajax({
			...
			success: function(respuesta) {
				...
				if (respuesta.mensaje == 0) {
					...
					$("#success-ingresar").html("Ingresar")
